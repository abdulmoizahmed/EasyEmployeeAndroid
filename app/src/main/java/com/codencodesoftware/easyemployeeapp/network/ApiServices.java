...
public class ApiServices {
    ...
    public static void postLogin(String username,String password,final OnResponseReceivedListener listener)
    {
        RestClient.getAuthAdapter().postLogin(username,password).enqueue(new Callback<RestResponse>() {
            @Override
            public void onResponse(Call<RestResponse> call, Response<RestResponse> response) {
                if(response.body() != null)
                    listener.onSuccessReceived(response.body());
